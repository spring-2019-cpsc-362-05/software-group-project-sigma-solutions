...
{
    getPlayersActive();
    getPlayersUserControlled();

    if (_playerIsActive) {
        game = new Game();
        game->show();
    } else
        qDebug() << "Please activate at least one player";
}
//Active States
...
void mainMenu::on_player7Active_stateChanged(int arg1) { _player[6].active = arg1;        qDebug() << "Player7A: " << arg1;   }
